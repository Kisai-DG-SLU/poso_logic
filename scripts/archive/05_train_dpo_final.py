    ...
    def patch_trainer():
        ...
        def patched_inner_loop(self, args, dataloader, *args2, **kwargs):
            ...
            if hasattr(self, "precompute_ref_log_probs") and hasattr(self, "beta"):
                ...
                # Remplacer la méthode get_batch_samples par notre version
                original_get_batch_samples = self.get_batch_samples
                
                # Définir une nouvelle méthode qui gère tous les cas
                def new_get_batch_samples(self, batch, num_batches=None, device=None):
                    # Si c'est un itérateur, prendre le premier élément
                    if hasattr(batch, "__iter__") and not isinstance(batch, dict) and not isinstance(batch, torch.Tensor):
                        try:
                            # C'est probablement un DataLoader ou un itérateur
                            real_batch = next(batch)
                            return original_get_batch_samples(self, real_batch)
                        except (StopIteration, TypeError) as e:
                            # Cas d'erreur, essayer quand même avec le batch original
                            print(f"WARNING: Erreur itération batch: {type(e).__name__}: {e}")
                            return original_get_batch_samples(self, batch)
                    
                    # Si device est spécifié mais pas num_batches (inversion de paramètres)
                    if num_batches is not None and device is None and isinstance(num_batches, (str, torch.device)):
                        # num_batches est en fait le device
                        return original_get_batch_samples(self, batch, num_batches)
                    
                    # Cas standard
                    return original_get_batch_samples(self, batch)
                
                # Remplacer la méthode
                self.get_batch_samples = new_get_batch_samples.__get__(self, type(self))
