"""
Version mini du script DPO avec solution finale
utilisant un patch complet du Trainer
"""
import sys, os
...
patch_module = SourceFileLoader("monkey_patch_direct_final", 
                                "/mnt/prod/scripts/monkey_patch_direct_final.py").load_module()
...
def get_dpo_config():
    ...
    return {
        ...
        "save_steps": 20,  # Réduit pour sauvegarder plus souvent
        "eval_steps": 20,  # Réduit pour évaluer plus souvent
        "output_dir": str(MODEL_DIR / "checkpoints" / "dpo_mini_test"),
        "save_total_limit": 1,
        "warmup_steps": 5,
        "max_samples": 50,  # Limiter encore plus le nombre d'exemples
        "max_steps": 10,  # Limiter le nombre d'étapes
    }
...
def train_dpo():
    ...
    training_args = TrainingArguments(
        ...
        eval_strategy="steps",  # Utiliser la même version que dans le script original
        save_strategy="steps",
    ...
    try:
        # Forcer l'usage de train() patché
...
def main():
    ...
    print("\n✅ TEST DPO RÉUSSI! Prêt pour l'entraînement complet.")
