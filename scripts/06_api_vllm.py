...
Note : nécessite vLLM installé (pip install vllm>=0.6.0 ou Docker)
"""
...
async def load_model():
    ...
    try:
        logger.info(f"Chargement du modèle vLLM: {BASE_MODEL} avec LoRA {ADAPTER_PATH}")
        llm = LLM(
            model=BASE_MODEL,
            enable_lora=True,
            max_model_len=2048,
        ...
        model_name_loaded = f"{BASE_MODEL}+LoRA"
...
async def root():
    return {
        ...
        "model": model_name_loaded or BASE_MODEL,
        "engine": "vLLM" if VLLM_AVAILABLE else "non disponible (pip install vllm ou Docker)",
    }
...
async def evaluate_triage(request: TriageRequest):
    if not VLLM_AVAILABLE or llm is None:
        raise HTTPException(status_code=503, detail="vLLM non disponible - utiliser Docker ou pip install vllm")
    ...
    try:
        ...
        outputs = llm.generate([prompt], sampling_params, lora_request=None)
...
if __name__ == "__main__":
    if not VLLM_AVAILABLE:
        logger.error("vLLM requis. Installez: pip install vllm>=0.6.0")
